...
// Disabilita timeout Express su ogni request
...
app.post("/api/:service", async (req, res) => {
  try {
    ...
    if (service === "chatbaseSimulateur") {
      ...
      // 2) Forza keep-alive e disabilita timeout TCP/HTTP sulla socket
      if (res.socket) {
        res.socket.setKeepAlive(true);
        res.socket.setTimeout(0);
      }

      // 3) Heartbeat per tenere viva la funzione Vercel
      const heartbeat = setInterval(() => {
        res.write(":\n\n");
      }, 4000);

      try {
        // 4) Chiedi lo stream a Chatbase
        ...
        // 5) Inoltra TUTTO lo stream così com’è al client
        cbRes.data.pipe(res);
        cbRes.data.on("end", () => {
    ...
    // Fallback generico (non dovrebbe essere mai raggiunto)
    ...
    if (error.code === 'ECONNABORTED' && req.params.service === "openaiAnalyse") {
...
// ✅ 📌 Endpoint per recuperare la chiave di OpenAI
...
// Avvia il server e disabilita il timeout Node.js
